...
import { Input, Checkbox, Select, SearchSelect, SubNav, PillToggle, GestureHelp } from "../components/ui";
import { useToast } from "../components/Toast";
import { searchProducts, iconFor } from "../lib/groceries";

type Tab = "liste" | "recettes" | "idees";
const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: "liste", label: "À acheter", icon: "🛒" },
  { id: "recettes", label: "Mes recettes", icon: "📖" },
  { id: "idees", label: "Idées repas", icon: "💡" },
];

export default function Courses() {
  const navigate = useNavigate();
  const { tab: tabParam, view: viewParam } = useParams();
  const tab: Tab = TABS.some((t) => t.id === tabParam) ? (tabParam as Tab) : "liste";

  return (
    <div className="space-y-4">
      <SubNav
        value={tab}
        onChange={(v) => navigate(`/courses/${v}`)}
        items={TABS.map((t) => ({ value: t.id, label: t.label, icon: t.icon }))}
      />
      {tab === "liste" && <ShoppingList />}
      {tab === "recettes" && <Recipes />}
      {tab === "idees" && <IdeasTab view={viewParam} />}
    </div>
  );
}

/* ---------------- Idées repas : sous-menus ---------------- */

function IdeasTab({ view }: { view?: string }) {
  const navigate = useNavigate();
  // Dernier sous-menu visité mémorisé (retour automatique dessus).
  const sub = useLastView("courses:idees", ["semaine", "nouvelles"], "semaine", view, "/courses/idees");
  return (
    <div className="flex flex-col gap-4">
      <PillToggle
        value={sub}
        onChange={(v) => navigate(`/courses/idees/${v}`)}
        items={[
          { value: "semaine", label: "Repas de la semaine", icon: "📅" },
          { value: "nouvelles", label: "Nouvelles recettes", icon: "✨" },
        ]}
      />
      {sub === "nouvelles" ? <MealIdeas /> : <WeekMealPlan />}
    </div>
  );
}
