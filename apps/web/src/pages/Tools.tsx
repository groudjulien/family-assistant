...
import { SubNav, Select, Input, Checkbox, DateTimeInput, DateRangeCalendar, PillToggle } from "../components/ui";
...
type Tab = "activites" | "films" | "vacances" | "wish";

const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: "activites", label: "Activités", icon: "🎲" },
  { id: "films", label: "Films", icon: "🎬" },
  { id: "vacances", label: "Vacances", icon: "🏖️" },
  { id: "wish", label: "WishList", icon: "⭐" },
];

export default function Tools() {
  const navigate = useNavigate();
  const { tab: tabParam } = useParams();
  const tab: Tab = TABS.some((t) => t.id === tabParam) ? (tabParam as Tab) : "activites";
...
function Activites() {
  ...
  const invalidateFav = () => qc.invalidateQueries({ queryKey: ["activity-favorites"] });
...
type FilmView = Audience | "vues" | "masques";
...
function Films() {
  const qc = useQueryClient();
  const [view, setView] = useState<FilmView>("enfants");
  // Fiche film ouverte (modale de détails), quel que soit le sous-onglet.
  const [detailId, setDetailId] = useState<string | null>(null);
  const audience: Audience = view === "adultes" ? "adultes" : "enfants";
  ...
  const { data: seenData } = useQuery({
    queryKey: ["films-seen"],
    queryFn: () => api.get<{ films: (Film & { audience?: string })[] }>("/api/films/seen"),
    enabled: view === "vues",
  });
  const { data: hiddenData } = useQuery({
    queryKey: ["films-hidden"],
    queryFn: () => api.get<{ films: Film[] }>("/api/films/hidden"),
    enabled: view === "masques",
  });

  // Recherche par titre (vues Enfants / Adultes) : debounce pour ne pas appeler
  ...
  const { data: searchData, isFetching: searchLoading } = useQuery({
    ...
    enabled: searching && view !== "vues" && view !== "masques",
    staleTime: 5 * 60 * 1000,
    retry: false,
  });
        ...
        <div className="flex flex-1 flex-col gap-1.5 p-3">
          ...
          <div className="mt-auto flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="text-lg leading-none text-slate-300 transition hover:text-brand-600"
            >
              👁
            </button>
            <button
  ...
  const seen = seenData?.films ?? [];
  const hidden = hiddenData?.films ?? [];

  return (
    <div className="space-y-4">
      <PillToggle
        value={view}
        onChange={(v) => setView(v as FilmView)}
        items={[
      ...
      {view === "vues" ? (
        <>
          <p className="text-xs text-slate-400">Films marqués comme déjà vus (exclus des propositions).</p>
          {seen.length === 0 ? (
            <div className="card text-sm text-slate-400">Aucun film vu pour l'instant.</div>
          ) : (
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">{seen.map(seenCard)}</div>
          )}
        </>
      ) : view === "masques" ? (
        <>
          <p className="text-xs text-slate-400">Films « ne plus me proposer » (en cas d'erreur, re-proposez-les).</p>
          {hidden.length === 0 ? (
            <div className="card text-sm text-slate-400">Aucun film masqué.</div>
          ) : (
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">{hidden.map(hiddenCard)}</div>
          )}
        </>
      ) : (
        <>
          ...
          <p className="text-xs text-slate-400">
            {searching
              ? "Résultats disponibles avec vos abonnements. ♡ = à regarder plus tard."
...
function Vacances() {
  ...
  return (
    <div className="space-y-4 pb-24 md:pb-0">
      {/* Sous-menus Prévu / Archivé — une URL par vue. Mobile : le select prend
          50 % et le bouton « Filtres » (recherche + période, en modale) l'autre 50 %. */}
      {/* Grille 1fr/auto/1fr : la bascule reste centrée dans la page, les actions
          restent à droite sans la décaler. */}
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        <span aria-hidden="true" />
        <PillToggle
          value={statut}
          onChange={(v) => navigate(`/tools/vacances/${v}`)}
          items={[
      ...
      {itemModal && targetTrip && (
        <ItemModal
          ...
          onSaved={() => {
            ...
            if (statut === "archive") navigate("/tools/vacances/prevu");
